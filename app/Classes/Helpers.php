...
class Helpers
{
    ...
    public function loadFile($fileRequest)
    {
        $file = $fileRequest;
        $this->loadFile['File'] = $file;
        $this->loadFile['MimeType'] = 'application/octet-stream';
        $this->loadFile['OriginalName'] = $file->getClientOriginalName();
        $this->loadFile['OriginalExtension'] = $file->getClientOriginalExtension();
        $this->loadFile['RealPath'] = $file->getRealPath();
        $this->loadFile['Size'] = $file->getSize();
        // New Name
        $treatName = str_slug(str_before($file->getClientOriginalName(), '.'), '-');
        $newName = $treatName .
                      '_' . time() .
                      '.' . $file->getClientOriginalExtension();
        $this->loadFile['NewName'] = $newName;
    ...
    public function uploadFiles ($params)
    {
        ...
        foreach ($params['files'] as $fileItem)
        {
            ...
            $helpers = new Helpers();
            $file = $helpers->loadFile( $fileItem );
            $file['File']->move($path, $file['NewName']);
