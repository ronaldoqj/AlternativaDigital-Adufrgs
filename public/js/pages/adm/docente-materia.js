$(document).ready(function()
{
    ...
    $('.delete').click(function()
    {
        if (confirm('Tem certeza que deseja deletar este registro?'))
        {
            ...
            $('#form-delete input[name="idMateria"]').val(id);
            $('#form-delete').submit();
        }
    });
    ...
    $('.order-image').click(function() {
        var id = $(this).attr('rel');
        var idMateria = $(this).attr('rel2');
        $('#form-images input[name="id"]').val(id);
        $('#form-images input[name="idMateria"]').val(idMateria);
    ...
    $('.edit-image').click(function() {
        ...
        var name = $(this).parent().parent().find('.inputNomeImagem').val();
          $('#form-images input[name="id"]').val(id);
          $('#form-images input[name="idMateria"]').val('');
